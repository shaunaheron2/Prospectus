...
abstract: "As community-based mental health services face increased demand for service, alongside significant workforce challenges, the ability to anticipate and optimize staff workload is critical to providing high quality care that is timely and fair in its allocation. Yet manual review of client records, necessary to fairly and efficiently manage clinician workload and allocate new clients is untenable in the face of the same workforce shortages. With this gap in mind, the goal of the current study is to evaluate the utility of leveraging electronic mental health records (EMHR) with machine learning models to anticipate and monitor the number of hours per week that individual clients contribute to a provider's work. Specific objectives include: (i) identify the best features to predict client-related work (caseweight) from structured demographic, administrative and assessment EMHRs; (ii) compare the predictive accuracy of several tree-based and neural network algorithms to predict client-related work; iii) compare the utility of modelling a continuous index of work (hours per week) compared to a classification of work intensity (i.e., low, medium, high) (iv) determine the potential for early and ongoing caseweight prediction to inform case assignment and workload management."
language: 
...
To this end, we propose to build and evaluate a mental health caseweight model reliant on EMHR data. Considering we want to predict resource use at regular stages in the client journey, only cases with a completed initial screener will be included in the analysis. Final counts after screening will be reported and added to the flowchart before analysis.
